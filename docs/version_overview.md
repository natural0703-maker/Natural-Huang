...
Line-break cleanup / continuous paragraph restoration has been integrated into the `convert` path in a minimal form. The current implementation is internal opt-in and disabled by default. Cleanup runs only on eligible ordinary paragraphs after output `.docx` generation and before save. This capability is not yet exposed as a formal CLI / GUI feature.
...
- Minimal internal line-break cleanup integration in `convert` (internal opt-in, default off).
- 獨立 Phase 1 CLI。
- legacy CLI 第一版保守轉發。
- 高風險規則預設檔名契約收斂。
...
- Line-break cleanup is not yet exposed through CLI / GUI.
- Line-break cleanup is not yet integrated with `apply_review`, paragraph merge, report, or GUI summary.
- Line-break cleanup still does not guarantee run-level formatting fidelity.
