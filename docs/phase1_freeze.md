...
## 19. Phase 2F-2 狀態
- Phase 2F-2：GUI paragraph merge diagnostics 顯示已完成。
- GUI 結果摘要區已固定顯示：
  - `段落合併 mismatch 總數`
  - `段落合併前段 mismatch`
  - `段落合併後段 mismatch`
  - `段落合併 mismatch 範例候選`
- diagnostics 區塊固定接在 paragraph merge summary 後面。
- analyze / convert 顯示預設值：
  - `0`
  - `0`
  - `0`
  - `無`
- `sample_candidate_ids` 有值時以 `id1, id2` 形式顯示。
- 不新增 GUI diagnostics controls。
- 不新增 reviewed JSON editor。
- 不修改 diagnostics 聚合邏輯。
- 不修改 paragraph merge apply 核心規則。

## 20. Phase 2F-3A 狀態

- Phase 2F-3A：diagnostics sample entries 結構已完成。
- `ParagraphMergeDiagnosticsPanelEntry` 已新增。
- `ParagraphMergeDiagnostics` 已新增 `sample_entries`。
- sample entries 只針對 paragraph merge 的 source mismatch 類型產生。
- `mismatch_type` 只區分：
  - `source_text`
  - `next_source_text`
- sample entries 最多保留前 3 筆，維持 encounter order。
- preview 規則固定為：
  - 換行壓成單一空白
  - 前後空白 strip
  - 最多 40 字元
...
- sample entries 不保存：
  ...
  - paragraph index 詳細清單
  - expected / actual 全文
- diagnostics count 與 paragraph merge summary 契約不變。
- 完整 pytest 已更新為：`243 passed`。
- `verify_v2.bat` / `verify_v2.ps1` 仍通過。

## 21. Phase 2F-3B 狀態

- Phase 2F-3B：GUI 唯讀 diagnostics sample entries 顯示已完成。
- GUI 結果摘要區已新增：
  - `段落合併 mismatch 範例：無`
  - 或最多 3 筆 sample entries
- 每筆 sample entry 顯示：
  - `candidate_id`
  - `mismatch_type`
  - `expected_preview`
  - `actual_preview`
- `mismatch_type` 顯示策略：
  - `source_text` → `前段 mismatch`
  - `next_source_text` → `後段 mismatch`
  - 未知值 → 保留原始值
- GUI 不對 preview 做二次加工，只顯示既有 preview。
- 本輪未實作：
  - diagnostics controls
  - reviewed JSON editor
  - 全文 diff
  - 完整 expected / actual 全文
  - paragraph index 詳細列表
  - 所有 candidate 明細
- 完整 pytest 已更新為：`244 passed`。
- `verify_v2.bat` / `verify_v2.ps1` 仍通過。

## 22. Phase G-2 狀態

- G-2：legacy entry governance 文件 / 使用說明收斂已完成。
- 目前入口定位固定為：
  - 建議 CLI 入口：`python -m src.phase1_cli`
  - GUI：薄包裝層，經 `src/gui/phase1_worker.py` 呼叫 pipeline
  - `src/cli_v35.py`：legacy compatibility forwarder
  - `app.py`：thin router / launcher
- 本輪只更新文件與使用說明，不修改 runtime 行為、不新增 CLI 參數、不刪除任何入口，也不做 migration / deprecation 承諾。

## 23. Phase G-3A 狀態

- G-3A：legacy forwarder wording / 註解 / 防回歸測試已完成。
- `src/cli_v35.py` 仍維持保守轉發條件：
  - `--json-report`
  - `--txt-report`
  - `--apply-review`
- `--reviewed-output` 不單獨觸發轉發。
- 本輪只補：
  - help / usage wording
  - `_should_forward_to_phase1(argv)` 附近註解
  - 防回歸測試
- 本輪未實作：
  - 擴張轉發條件
  - 新增 CLI 參數
  ...
  - 在正常執行路徑新增 warning
  - `app.py` runtime 治理調整
- 完整 pytest 已更新為：`292 passed`。
- `verify_v2.bat` / `verify_v2.ps1` 仍通過。

## Phase I-4E 狀態

- 已完成「段內斷行清理／連續段落還原」的最小 `convert` 主流程接入。
- 目前設計為 internal opt-in，預設值為 `False`。
- 在輸出 `.docx` 完成主要文字轉換後、存檔前，僅針對 eligible ordinary paragraph 嘗試 cleanup。
- 保守跳過：heading、table cell、list-like、dialogue-like、English-dominant、special Word structure。
- 本階段未新增 CLI / GUI 開關。
- 本階段未接入 `apply_review`。
- 本階段未接入 paragraph merge。
- 本階段未接入 report / GUI summary。
- 本階段未承諾 run-level formatting fidelity。
- 本階段未預設啟用 cleanup。

## 24. 明確未納入 Phase 1 的項目
