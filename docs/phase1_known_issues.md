...
Phase 2E-1 已完成 `paragraph_merge_candidates` 的 schema / reviewed JSON 契約收斂；但 paragraph merge apply 仍未實作，尚未進行 `PARAGRAPH_MERGE_SOURCE_MISMATCH` 實際比對，也尚未建立 paragraph merge 結果摘要、GUI / report 的 merge 狀態顯示。
