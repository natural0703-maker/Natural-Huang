...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="將 DOCX 正文簡體中文轉為繁體，並輸出處理結果與報告。",
    )
    ...
    input_group.add_argument("--input-dir", required=False, help="資料夾輸入路徑")
    parser.add_argument(
        "--recursive",
        action="store_true",
        help="遞迴掃描 --input-dir 底下子資料夾中的 .docx",
    )
    parser.add_argument("--output-dir", required=True, help="輸出資料夾路徑")
    parser.add_argument("--config", required=False, help="設定檔 YAML 路徑")
    parser.add_argument("--term-dict", required=False, help="低風險詞庫 YAML 路徑")
    parser.add_argument("--report-name", required=False, help="Excel 報告檔名")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        try:
            ...
            LOGGER.exception("寫入 Excel 報告失敗。")
            print("錯誤：寫入 Excel 報告失敗。", file=sys.stderr)
            return 1
